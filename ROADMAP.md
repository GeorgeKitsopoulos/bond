...
- Stage 2G-D adds the fourth safe slice of that track with a read-only package-manager classification and dependency planning contract and implements only the fourth sequence item (package manager strategy classification and dependency planning).
...
- coverage now also includes Stage 2G-A host portability profile, Stage 2G-B storage portability hardening, and Stage 2G-C install manifest/drift detection checks.
- coverage now also includes Stage 2G-D package-manager strategy classification and dependency planning contract checks, Stage 2G-D forward-fix regression checks for host_profile distro_like resolution, rpm-ostree non-host-package claims, and manual-review next-step aggregation, and Stage 2G-D second forward-fix checks for manual-review item status alignment and top-level aggregation consistency.
- coverage now also includes Stage 2G-D package-manager strategy classification and dependency planning checks.
...
- coverage now also includes Stage 2G-F-J final user-space install readiness report checks for shape/boundaries, final-review-ready status mapping, deterministic readiness digest/JSON preview behavior, missing-input blocking, disabled-executor invariant rejection, chain-closure flags, non-executing formatting, read-only probe shape, and docs/source boundary enforcement.
...
- Stage 2G-D implements only item 4.
- Stage 2G-E implements only item 5.
- Stage 2G-F-A begins item 6 by adding a non-executing user-space install write-set contract.
- Stage 2G-F-B continues item 6 by adding a non-executing user-space install manifest payload contract.
...
- Stage 2G-F-J does not complete the user-space installer, remains non-executing, does not collect approval, does not validate approval as true, does not authorize execution or writes, does not perform real installer writes, and does not add CLI surface.
- Controlled dependency installer, updater, service backend abstraction, and Steam Deck/Bazzite satellite runtime are still not implemented.
