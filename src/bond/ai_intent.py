...
ACTION_START_PATTERNS = [
    ...
    r"^\s*create\s+(a\s+)?folder\b",
    r"^\s*make\s+(a\s+)?folder\b",
    r"^\s*write\s+(a\s+)?file\b",
    r"^\s*write\s+.*\s+in\s+\w+\b",
    ...
    r"^\s*upgrade\s+all\s+system\s+packages\b",
    r"^\s*upgrade\s+(my\s+)?packages\b",
    r"^\s*update\s+(my\s+)?packages\b",
    r"^\s*run\s+rm\s+-rf\b",
    r"^\s*ανοιξε\b",
    r"^\s*άνοιξε\b",
    r"^\s*δειξε\b",
