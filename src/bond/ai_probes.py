...
from ai_user_install_readiness import build_user_install_readiness_report
...
def run_named_probe(name: str) -> ProbeResult:
    ...
    dispatch = {
        ...
        "user_install_readiness_report": probe_user_install_readiness_report,
    }
...
def probe_user_install_approval_plan() -> ProbeResult:
    """Read-only user-space install approval-envelope planning probe."""
    try:
        user_install_transaction_result = probe_user_install_transaction_plan()
        user_install_transaction_plan = (
            user_install_transaction_result.data
            if user_install_transaction_result.ok
            else {}
        )

        plan = build_user_install_approval_plan(
            user_install_transaction_plan=user_install_transaction_plan,
            requested_mode="doctor_review",
        )
        ...
        return probe_ok(
            ...
            data=plan,
            notes=(
                "Read-only user-space install approval-envelope probe; it does not grant approval, "
                "create directories, write manifests, run commands, mutate services, install packages, "
                "or move storage."
            ),
        )
    except Exception as exc:
...
def probe_user_install_review_report() -> ProbeResult:
    """Read-only user-space install review/report probe."""
    try:
        user_install_execution_gate_result = probe_user_install_execution_gate()
        user_install_execution_gate = (
            user_install_execution_gate_result.data
            if user_install_execution_gate_result.ok
            else {}
        )

        report = build_user_install_review_report(
            user_install_execution_gate=user_install_execution_gate,
            requested_mode="doctor_review",
        )

        return probe_ok(
            probe_name="user_install_review_report",
            layer=3,
            source_type=SOURCE_RUNTIME_PROBE,
            certainty_class=CERTAINTY_DERIVED,
            refresh_class=REFRESH_MEDIUM_CHURN,
            ...
            data=report,
            notes=(
                "Read-only user-space install review/report probe; it does not validate approval, "
                "authorize execution, create directories, write manifests, run commands, mutate services, "
                "install packages, or move storage."
            ),
        )
...
def probe_user_install_write_preflight() -> ProbeResult:
    """Read-only user-space install write-preflight probe."""
    try:
        user_install_review_result = probe_user_install_review_report()
        user_install_review_report = (
            user_install_review_result.data
            if user_install_review_result.ok
            else {}
        )

        report = build_user_install_write_preflight(
            user_install_review_report=user_install_review_report,
        ...
        return probe_ok(
            ...
            data=report,
            notes=(
                "Read-only user-space install write-preflight probe; it does not validate approval, "
                "authorize execution, create directories, write manifests, run commands, mutate services, "
                "install packages, or move storage."
            ),
        )
...
def probe_user_install_readiness_report() -> ProbeResult:
    """Read-only final user-space install readiness report probe."""
    try:
        user_install_write_executor_result = probe_user_install_write_executor()
        user_install_write_executor = (
            user_install_write_executor_result.data
            if user_install_write_executor_result.ok
            else {}
        )

        report = build_user_install_readiness_report(
            user_install_write_executor=user_install_write_executor,
            requested_mode="doctor_readiness",
        )

        return probe_ok(
            probe_name="user_install_readiness_report",
            layer=3,
            source_type=SOURCE_RUNTIME_PROBE,
            certainty_class=CERTAINTY_DERIVED,
            refresh_class=REFRESH_MEDIUM_CHURN,
            ...
            data=report,
            notes=(
                "Read-only final non-executing user-space install readiness aggregation; "
                "it does not collect approval, validate approval, authorize execution, create directories, "
                "write manifests, run commands, mutate services, install packages, or move storage."
            ),
        )
    except Exception as exc:
