...
# Stage 2F-F-C/D cross-reference: build_maintenance_plan and the
# Non-executing maintenance plan: section are now assembled in ai_maintenance_report.
# Stage 2F-F-B/D cross-reference: probe calls moved to ai_maintenance_report:
#   run_named_probe("package_update_status"), run_named_probe("storage_hygiene"),
#   run_named_probe("boot_service_health") are all invoked there.
from ai_maintenance_report import build_and_format_maintenance_readiness_report
...
def _build_maintenance_readiness_report() -> str:
    return build_and_format_maintenance_readiness_report()
