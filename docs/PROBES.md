...
### Stage 2G-F-A user-space install write-set planning
...
### Stage 2G-F-C user-space install transaction/preflight planning

- `user_install_transaction_plan` is a read-only user-space install transaction/preflight planning probe for future installer/updater/satellite planning.
- It composes existing `user_install_plan` and `user_install_manifest_plan` into a read-only transaction/preflight preview.
- It does not create directories.
- It does not write manifests.
- It does not run commands.
- It does not mutate services.
...
- `user_install_approval_validation` composes existing `user_install_write_preflight` output into a deterministic user-space install approval-validation contract and challenge.
- Layer: 3.
- Source: `SOURCE_RUNTIME_PROBE`.
- Certainty: `CERTAINTY_DERIVED`.
- Refresh: `REFRESH_MEDIUM_CHURN`.
- It derives deterministic `approval_challenge` and `approval_challenge_json_preview` outputs for future user-space install approval-validation review.
...
- `user_install_write_executor` composes existing `user_install_approval_validation` output into a deterministic disabled/default-deny user-space install write-executor packet.
- Layer: 3.
- Source: `SOURCE_RUNTIME_PROBE`.
- Certainty: `CERTAINTY_DERIVED`.
- Refresh: `REFRESH_MEDIUM_CHURN`.
- It derives deterministic `executor_disabled_packet` and `executor_json_preview` outputs for future user-space install executor review.
- It does not collect approval.
- It does not validate approval.
- It does not authorize execution.
- It does not authorize writes.
