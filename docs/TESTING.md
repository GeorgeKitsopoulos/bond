...
Coverage now also includes Stage 2G-D second forward-fix regression checks:
...
Coverage now also includes Stage 2G-F-C user-space install transaction/preflight planning checks:

- `stage2g_f_c_user_install_transaction_shape_and_boundaries`
- `stage2g_f_c_user_install_transaction_operation_order_is_deterministic`
- `stage2g_f_c_user_install_transaction_requires_manifest_path_alignment`
...
- `package_update_status` shape
- `storage_hygiene` shape
...
Stage 2F-F-E report-readiness-metadata coverage includes:

- report readiness metadata shape
- limited-signal report readiness metadata behavior
- formatted report readiness metadata markers
- source and documentation boundary guard
